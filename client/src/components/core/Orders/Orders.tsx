...
const Orders: React.FC = () => {
  ...
  return (
    <Box m="60px auto 90px" p="0 16px" maxWidth="900px">
      ...
      {state.noOrders ? (
        <Box fontSize="h4.fontSize">У вас нет заказов</Box>
      ) : (
        <>
          <FormControl
          ...
          >
            <InputLabel htmlFor="searchbox">
              Имя продукта или идентификатор заказа
            </InputLabel>
            <OutlinedInput
              id="searchbox"
              name="Search"
              label="Имя продукта или идентификатор заказа"
              fullWidth
              value={value}
          ...
          {state.loading || state.orders.length > 0 ? (
            ...
            <Box fontSize="h4.fontSize">Ничего не найдено.</Box>
          )}
        </>
      )}
    </Box>
