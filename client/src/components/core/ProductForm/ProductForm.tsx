...
const ProductForm: React.FC = () => {
    ...
    () => {
      if (slug) {
        getProduct(slug, cancelSource.current?.token)
          .then((response) => {
            const { categories, price, quantity, slug, ...otherFields } =
              response.data;
            setFormValues({
              ...otherFields,
  ...
  const onSubmit = (
    ...
    const { name, description, category, brand, price, quantity } =
      formikValues;
  ...
  return (
    <FormLayout
      title={slug ? "Обновить продукт" : "Добавить продукт"}
      maxWidth={700}
      error={state.error}
          ...
          {({ submitForm, isSubmitting, isValid, dirty, values }) => (
            <Form>
              ...
              <CustomButton
                ...
                onClick={submitForm}
              >
                {slug ? "Обновить продукт" : "Добавить продукт"}
              </CustomButton>
            </Form>
          )}
        </Formik>
