      ...
      getCartItems(cancelSource.current?.token)
        ...
        .catch((err) => {
          if (!axios.isCancel(err)) {
            showSnackbar("Не удалось загрузить товары", "error");
  ...
  const changeProductQuantity = (item: CartItemValues, i: number) => {
    ...
    addOneToCart(item, cancelSource.current.token)
      .then((response) => {
        ...
        showSnackbar("Количество продуктов обновлено", "success");
      })
      .catch((err) => {
        if (!axios.isCancel(err)) {
          ...
          showSnackbar("Не получилось обновить количество", "error");
        }
      });
  };
  ...
  const removeItem = (i: number) => {
    ...
    removeCartItem(
      ...
      .then((response) => {
        ...
        setCartItems({ ...cartItems });
        setOrderSummary(calculateOrderSummary(cartItems.items));
        showSnackbar("Товар удален из корзины", "success");
      })
      .catch((err) => {
