...
const Shop: React.FC = () => {
  ...
  const initialQuery: SearchParams = useMemo(() => {
    ...
    if (brandSlug) {
      query.brand = [
        brands.find((brand) => brand.slug === brandSlug)?._id || "id not found",
      ];
    }
    if (keywords) {
    ...
    return query;
  }, [categorySlug, keywords, brandSlug, categories, brands]);
            ...
            setFilterOptions({
              ...
              brands:
                trigger === "price" || trigger === "pathname"
                  ? results.brands.map(({ _id, count, name, slug }) => ({
                      id: _id,
                      name: name,
                      slug: slug,
                      count,
  ...
  const handleBrandToggle = (brand: Brand) => {
    const index = filterValues.brand.indexOf(brand.slug);
    if (index >= 0) {
      filterValues.brand.splice(index, 1);
    } else {
      filterValues.brand.push(brand.slug);
    }
    const newQuery: SearchParams = {
      ...query,
      brand: [
        ...brands
          .filter((brand) =>
            filterValues.brand.find((slug) => slug === brand.slug)
          )
          .map((brand) => brand._id),
      ],
    };
                        ...
                        <FormControlLabel
                          ...
                          label={
                            priceRange.from < 3000
                              ? `${priceRange.from} BYN  -  ${priceRange.to} (${priceRange.count}) BYN`
                              : `${priceRange.from} и выше (${priceRange.count})`
                          }
                        />
                      ))}
                    </RadioGroup>
