  ...
  const addProductToWishlist = () => {
    ...
    addToWishlist(product?.slug as string)
      .then((response) => {
        ...
        if (exists) {
          showSnackbar("Товар уже в желаемом", "info");
        } else {
          showSnackbar("Товар успешно добавлен в желаемое", "success");
        }
        setIsSubmitting(false);
      })
      .catch((err) => {
    ...
    <>
      <Box p="60px 4vw 70px" bgcolor="#fff">
        ...
        <TabContext value={tabValue}>
          ...
          >
            <Tab className={classes.tab} label="Описание" value="overview" />
            <Tab
            ...
            )}
            {
              <div className={classes.reviewButtonContainer}>
                <CustomButton
                  ...
                  color="primary"
                >
                  {reviews?.myReview ? "Редактировать отзыв" : "Написать отзыв"}
                </CustomButton>
                {reviews?.myReview && (
                  <CustomButton
                    color="primary"
      ...
      <Box p="40px 0 90px">
        <ProductSlider title="Так же может понравиться" products={relatedProducts} />
      </Box>
    </>
  );
};
